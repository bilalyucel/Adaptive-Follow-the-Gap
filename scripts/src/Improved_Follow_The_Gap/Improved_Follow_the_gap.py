		...
		while not rospy.is_shutdown():		
				...
				if need:
					...
					for x in range(len(self.obj_list)):
						...
						if self.obj_list[x]<320:
							...
							if len(self.thislist)>self.obj_list[x]:
			...
			if self.state==4:
				if self.poseCorrect:
					...
					'''
					if (math.sqrt(self.x*self.x+self.y*self.y)>math.sqrt(xgoal*xgoal+ygoal*ygoal)) and self.avoidObs and (self.theta>-math.pi and self.theta<math.pi) :
						self.avoidObs=False
